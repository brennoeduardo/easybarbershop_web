    ...
    <v-app>
      <div class="container">
        <div class="show-error">
          <h1 class="colorBlue">{{ error.statusCode }}</h1>
        </div>
        <div class="stack-container">
          <div class="card-container">
            <div
              class="perspec"
              style="--spreaddist: 125px; --scaledist: 0.75; --vertdist: -25px"
            >
              <div class="card">
                <div class="writing">
                  <div class="topbar">
                  ...
                  <div class="code">
                    <ul v-if="error.statusCode == 404">
                      Erro ao encontrar página, verifique se o endereço está correto.
                    </ul>
                    <ul v-else>
                      {{
                        error.message
...
const pageNotFound = ref("Atlas - 404 Not Found");
const otherError = ref("Atlas - Error");
const parsedError = ref(null);
const intentionalError = ref(false);

onMounted(() => {
  parsedError.value = JSON.parse(JSON.stringify(props.error));
});

const sendEmailToDev = () => {
  // try {
  //   await this.$axios.$post('/emails/send/errorMail', {
  ...
  //   this.$toast.error('Erro ao enviar e-mail para a equipe responsável.')
  // }
};
</script>
